...
def buscar_reservas(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            basic_count = habitaciones_disponibles.filter(id_tipo_habitacion__nombre='Basic').count()
            premium_count = habitaciones_disponibles.filter(id_tipo_habitacion__nombre='Premium').count()
            suit_count = habitaciones_disponibles.filter(id_tipo_habitacion__nombre='Suit').count()
